...
cupdlp_bool PDHG_Check_Termination_Min_Requirments(CUPDLPwork* pdhg, int bool_print, float min_tol) {
    ...
    int bool_pass =
        ((resobj->dPrimalFeas <
            min_tol * (1.0 + scaling->dNormRhs)) &&
            (resobj->dDualFeas < min_tol * (1.0 + scaling->dNormCost)) &&
            (resobj->dRelObjGap < min_tol));
    return bool_pass;
}

cupdlp_bool PDHG_Check_Termination_Min_Requirments_Average(CUPDLPwork* pdhg, int bool_print, float min_tol) {
    CUPDLPproblem* problem = pdhg->problem;
    CUPDLPsettings* settings = pdhg->settings;
    CUPDLPresobj* resobj = pdhg->resobj;
    CUPDLPscaling* scaling = pdhg->scaling;
    ...
    int bool_pass =
        ((resobj->dPrimalFeasAverage <
            min_tol * (1.0 + scaling->dNormRhs)) &&
            (resobj->dDualFeasAverage < min_tol * (1.0 + scaling->dNormCost)) &&
            (resobj->dRelObjGapAverage < min_tol));
    return bool_pass;
}
    ...
    for (timers->nIter = 0; timers->nIter < settings->nIterLim; ++timers->nIter) {
        ...
        if (bool_checking) {
            ...
            if (PDHG_Check_Termination(pdhg, bool_print)) {
                ...
                //cupdlp_printf("%27s %+15.8e\n", "Gap:", resobj->dPrimalObj-resobj->dDualObj);
            ...
            if (PDHG_Check_Termination_Average(pdhg, bool_print)) {
                ...
                //cupdlp_printf("%27s %+15.8e\n", "Gap:", resobj->dPrimalObjAverage - resobj->dDualObjAverage);
                //cupdlp_printf("%27s %8.2e / %8.2e\n",
            ...
            if (timers->dSolvingTime > settings->dTimeLim) {
                ...
                if (PDHG_Check_Termination_Min_Requirments(pdhg, 0, 1e-3)) {
                    resobj->termIterate = LAST_ITERATE;
                    resobj->termCode = TIMELIMIT_OR_ITERLIMIT;
                    break;
                }
                if (PDHG_Check_Termination_Min_Requirments_Average(pdhg, 0, 1e-3)) {
                    CUPDLP_COPY_VEC(iterates->x->data, iterates->xAverage->data,
                        cupdlp_float, problem->nCols);
                    CUPDLP_COPY_VEC(iterates->y->data, iterates->yAverage->data,
                        cupdlp_float, problem->nRows);
            ...
            if (timers->dSolvingTime > settings->dTimeLim * 2) {
                cupdlp_printf("Time limit reached and 1e-3 tol is not reached.\n");
                PDHG_Check_Termination_Min_Requirments(pdhg, 1, 1e-3);
                PDHG_Check_Termination_Min_Requirments_Average(pdhg, 1, 1e-3);
                resobj->termCode = TIMELIMIT_OR_ITERLIMIT;
                break;
            }
